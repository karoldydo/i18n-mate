-- =====================================================================
-- migration: fix create_project_locale_atomic return type mismatch
-- description: Fix "structure of query does not match function result type" error
-- problem: The function return type declaration doesn't match the actual column types
-- solution: Align return type with actual table column types or fix casting
-- =====================================================================
...
BEGIN
  ...
  IF v_key_count > 0 THEN
    ...
    IF v_actual_translations = 0 THEN
      -- Rollback the locale creation (use table alias to avoid ambiguity)
  ...
  -- Return the created locale with explicit column references and proper casting
  RETURN QUERY
  SELECT
    pl.id,
    pl.project_id,
    pl.locale::TEXT,  -- Explicit cast from locale_code to TEXT
    pl.label::TEXT,   -- Explicit cast to ensure TEXT type
    pl.created_at,
    pl.updated_at
...
EXCEPTION
  -- Catch our custom exceptions and re-raise them as-is
  WHEN SQLSTATE '42501' THEN  -- Authentication/Authorization errors
    RAISE;
  WHEN SQLSTATE '23505' THEN  -- Unique violation
    RAISE;
  WHEN SQLSTATE '23514' THEN  -- Check constraint violation
    RAISE;
  WHEN SQLSTATE '50000' THEN  -- Custom application errors
    RAISE;
