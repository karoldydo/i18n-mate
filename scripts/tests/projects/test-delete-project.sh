...
# Test: Delete Project
# Tests useDeleteProject hook via DELETE on projects table
...
# Delete project
print_step "Delete Project"

print_info "PROJECT_ID: ${PROJECT_ID}"
echo -e "${YELLOW}⚠ This will permanently delete the project and all related data${NC}"

DELETE_RESPONSE=$(curl -s -X DELETE "${API_BASE}/projects?id=eq.${PROJECT_ID}" \
...
sleep 1
VERIFY_RESPONSE=$(curl -s -X GET "${API_BASE}/projects?id=eq.${PROJECT_ID}&select=*" \
  -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "apikey: ${SUPABASE_ANON_KEY}" \
  -H "Content-Type: application/json")
