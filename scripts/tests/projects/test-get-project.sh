...
# Tests useProject hook via direct projects table query
...
# Get project
print_step "Get Project Details"

print_info "PROJECT_ID: ${PROJECT_ID}"

GET_RESPONSE=$(curl -s -X GET "${API_BASE}/projects?id=eq.${PROJECT_ID}&select=*" \
...
PROJECT_NAME=$(echo "${GET_RESPONSE}" | grep -o '"name":"[^"]*"' | head -1 | cut -d'"' -f4)
