...
# Test: List Keys in Default Language View
# Tests useKeysDefaultView hook via list_keys_default_view RPC
...
if [ -z "$1" ]; then
    ...
    echo "Usage: $0 <PROJECT_ID> [SEARCH_QUERY] [MISSING_ONLY]"
    exit 1
fi

PROJECT_ID="$1"
SEARCH_QUERY="${2:-}"
MISSING_ONLY="${3:-false}"
...
# List keys in default view
print_step "List Keys in Default Language View"
...
LIST_RESPONSE=$(curl -s -X POST "${RPC_BASE}/list_keys_default_view" \
...
print_success "Retrieved ${KEY_COUNT} key(s)"
