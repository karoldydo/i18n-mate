...
# Test: Create Project Locale
# Tests useCreateProjectLocale hook via create_project_locale_atomic RPC
...
# Create locale
print_step "Create Project Locale"

LOCALE_CODE="pl"
LOCALE_LABEL="Polski"

print_info "PROJECT_ID: ${PROJECT_ID}"
print_info "Locale Code: ${LOCALE_CODE}"
print_info "Locale Label: ${LOCALE_LABEL}"

CREATE_RESPONSE=$(curl -s -X POST "${RPC_BASE}/create_project_locale_atomic" \
  ...
  -d "{
    ...
    \"p_label\": \"${LOCALE_LABEL}\"
  }")

echo -e "\nResponse:"
echo "${CREATE_RESPONSE}" | python3 -m json.tool 2>/dev/null || echo "${CREATE_RESPONSE}"

LOCALE_ID=$(echo "${CREATE_RESPONSE}" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
...
print_success "Locale created successfully"
print_info "LOCALE_ID: ${LOCALE_ID}"

# Export for other scripts
echo ""
echo "export LOCALE_ID=${LOCALE_ID}"
