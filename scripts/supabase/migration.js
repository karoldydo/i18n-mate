...
async function processProject(projectRef, index, total) {
  ...
  const confirmed = await askConfirmation(
    `Do you want to proceed with migrations and deployment for project "${trimmedRef}"?`
  );
  ...
  log.step('Listing current migrations');
  const listResult = executeCommand('npx supabase migration list', { silent: true });
  if (listResult.success) {
    log.success('Migrations listed successfully');
    if (listResult.output) {
      console.log(listResult.output);
    }
    results.steps.push({ name: 'list', output: listResult.output, success: true });
  } else {
    log.error(`Failed to list migrations: ${listResult.error}`);
    results.success = false;
    results.steps.push({ error: listResult.error, name: 'list', success: false });
    return results;
  }

  // step 3: push migrations
  log.step('Pushing database migrations');
  const pushResult = executeCommand('npx supabase db push');
  if (!pushResult.success) {
    log.error(`Failed to push migrations: ${pushResult.error}`);
    results.success = false;
    results.steps.push({ error: pushResult.error, name: 'push', success: false });
    return results;
  }
  log.success('Migrations pushed successfully');
  results.steps.push({ name: 'push', success: true });

  // step 4: deploy functions
  log.step('Deploying Edge Functions');
  const deployResult = executeCommand('npx supabase functions deploy');
  if (!deployResult.success) {
    log.error(`Failed to deploy functions: ${deployResult.error}`);
    results.success = false;
    results.steps.push({ error: deployResult.error, name: 'deploy', success: false });
    return results;
  }
  log.success('Functions deployed successfully');
  results.steps.push({ name: 'deploy', success: true });
